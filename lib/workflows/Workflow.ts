...
export class Runner extends Base {
  ...
  //       sources.push(
  //         sourceValues.get(`${source}.tracks`) as SpotifyApi.PlaylistTrackObject
  //       );
  //     } else {
  ...
  async runWorkflow(workflow: WorkflowObject, timeoutAfter = 20000) {
    ...
    const workflowPromise = new Promise(async (resolve, reject) => {
      try {
        ...
        const sourceValues = new Map() as Map<string, any>;
        sourceValues.set("sourceValues key", "sourceValues value");
